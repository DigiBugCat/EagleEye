...
server.registerTool(
  "capture_gaze",
  {
    title: "Capture gaze",
    description:
      "Capture one user-approved EagleEye screen region centered on the calibrated gaze estimate. The result contains the exact annotated JPEG with dimensions and SHA-256; a top-left image coordinate system; pixel and normalized gaze coordinates; confidence, estimator, sample count, fixation duration and freshness when available; two-axis uncertainty with normalized radii and clipped bounds; crop role, provenance, relationships, padding, clipping and user-adjustment state; and optional provider enrichment labeled as untrusted advisory data. This is useful for understanding what is under or near the user's gaze or inspecting screen context through EagleEye. It is a one-shot capture and may return structured availability, permission, connection, calibration, approval or capture errors.",
    inputSchema: {
